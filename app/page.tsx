...
// University campus demo — realistic multi-building layout
...
export default function Home() {
  ...
  const loadDemo = () => {
    setNodes(DEMO_NODES); setEdges(DEMO_EDGES); setSelectedNode(null);
    toast("Demo campus loaded");
  };

  // Add a scenario template to the existing graph
  const addScenario = useCallback((scenarioIdx: number) => {
    ...
    const newEdges: GraphEdge[] = scenario.edges.map(e => ({
      id: eid(),
      source: idMap[e.sourceIdx],
      target: idMap[e.targetIdx],
      relation: e.relation,
    }));
...
{
  "nodes": [{"id":"n1","label":"string","type":"building|meter|hvac|equipment|grid|renewable|sensor|program|chiller_plant|distribution|storage|ems","properties":{"key":"value"}}],
  "edges": [{"id":"e1","source":"n1","target":"n2","relation":"FEEDS|MONITORS|SERVES|CONTROLS|MEASURES|PART_OF|CONNECTS_TO|ADJACENT_TO|REPORTS_TO|OPTIMIZES|CHARGES|DISPATCHES|INTERLOCKS_WITH"}]
}
Use 6–12 nodes and 6–12 edges. Labels should be specific and realistic for a university campus energy system. Include meaningful engineering properties (capacity, protocol, vintage, etc.).`,
  ...
  const meterCount = nodes.filter(n => n.type === "meter").length;
      ...
      <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 20, pointerEvents: "none" }}>
            <Stat label="Buildings" value={buildingCount} color="#00D4FF" />
            <Stat label="Meters" value={meterCount} color="#F59E0B" />
            <Stat label="Total Nodes" value={nodes.length} color="#8B9BB4" />
            <Stat label="Relationships" value={edges.length} color="#8B9BB4" />
          </div>
        ...
        {nodes.length === 0 && (
          <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", textAlign: "center", pointerEvents: "none" }}>
            ...
            <div style={{ fontSize: 13, color: "#1E2A3A", marginTop: 6 }}>Add buildings from the panel, use a template, or generate with AI</div>
          </div>
        )}

        <div style={{ position: "absolute", bottom: 14, right: 16, fontSize: 10, color: "#2A3A4A", display: "flex", gap: 12 }}>
          <span>Scroll to zoom</span><span>Drag to pan</span><span>Click node to select</span>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ fontSize: 12, color: "#4B5563", marginBottom: 16, lineHeight: 1.6 }}>
              Describe a campus energy system scenario. New nodes are appended to the existing graph.
            </p>
            ...
            <div style={{ marginBottom: 8 }}>
              ...
              <textarea value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} rows={4}
                placeholder="e.g. Map the steam tunnel distribution network from the central boiler plant to the six oldest buildings on the north quad, including trap stations and BTU meters…"
